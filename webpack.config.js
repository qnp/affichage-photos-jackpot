...
const sharedConfig = require('./src/client/shared/config');
...
const SOURCES_PATH = path.resolve(ROOT, 'src/client');
const ENTRY_POINT = path.resolve(SOURCES_PATH, 'game');
const OUTPUT_PATH = path.resolve(ROOT, 'dist');
const MODULES_PATHS = [path.resolve(ROOT, 'node_modules'), SOURCES_PATH];
const PUBLIC_PATH = '';
...
const config = {
  ...
  plugins: [
    ...
    new HtmlWebpackPlugin({
      title: 'Affichage Photos Jackpot',
      cache: false,
  ...
  module: {
    rules: [
      ...
      { test: /\.(frag|vert|log)$/, loader: 'raw-loader', include: INCLUDE_PATHS },
      { test: /\.jpe?g$|\.svg$|\.png$/, exclude: EXCLUDE_PATHS, loader: "file-loader?name=[path][name].[ext]"},
    ],
  },
  devServer: {
